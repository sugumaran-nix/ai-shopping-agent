...
`Product.validated()` is the gate every scraped item must pass before it is
...
class Product(BaseModel):
    source: Source
    title: str
    price: float = Field(gt=0, lt=10_000_000)  # upper bound added
    ...
    def image_url_valid(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v and not v.startswith(("http://", "https://", "//")):
            return None  # silently drop bad image URLs rather than reject the product
        return v or None
