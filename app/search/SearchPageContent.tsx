...
export default function SearchPageContent() {
  ...
  const runSearch = useCallback(async (query: string) => {
    ...
    try {
      // searchProducts takes a plain string — NOT an object
      const result = await searchProducts(q, ctrl.signal);
      setData(result);
      setSelected(result.results.filter(r => r.products.length > 0).map(r => r.source));
    } catch (e) {
  ...
  const filtered  = selected.length ? allFlat.filter(p => selected.includes(p.site)) : allFlat;
    ...
    <div className="min-h-screen flex flex-col">
      <header className="sticky top-0 z-30 glass border-b border-white/[0.05]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center gap-3">
          ...
          >
            <ArrowLeft className="w-4 h-4" style={{ color: "rgba(240,240,255,0.6)" }} />
          </button>
          <div className="flex-1">
            <SearchBar defaultValue={params.get("q") ?? ""} loading={loading} onSearch={runSearch} />
          </div>
        </div>
      </header>

      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-6 flex flex-col gap-5">
        {loading && <><AnalysisSkeleton /><GridSkeleton count={10} /></>}

        {!loading && error && (
          <ErrorState message={error} onRetry={() => runSearch(params.get("q") ?? "")} />
        )}

        {!loading && !error && data && (
          <AnimatePresence mode="wait">
            <motion.div key={data.query} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-5">
              <AIAnalysis recommendation={data.ai_recommendation} error={data.ai_error} staleSources={staleSources} />
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {data.results.map(r => (
                    <button
                      ...
                      style={{
                        background: selected.includes(r.source) ? (SITE_META[r.source as Source]?.bg ?? "rgba(124,58,237,0.12)") : "rgba(255,255,255,0.04)",
                        border: `1px solid ${selected.includes(r.source) ? (SITE_META[r.source as Source]?.color ?? "#7C3AED") + "45" : "rgba(255,255,255,0.07)"}`,
                        color: selected.includes(r.source) ? (SITE_META[r.source as Source]?.color ?? "#A78BFA") : "rgba(240,240,255,0.5)",
                      }}
                  ...
                  <select
                    ...
                    style={{ color: "rgba(240,240,255,0.8)", border: "1px solid rgba(255,255,255,0.08)" }}
              ...
              {!hasProds ? (
                <EmptyState query={data.query} onReset={() => router.push("/")} onSearch={runSearch} />
              ) : displayed.length === 0 ? (
                <div className="text-center py-12 text-sm" style={{ color: "rgba(240,240,255,0.6)" }}>
                  No results for selected stores.{" "}
                  <button type="button" className="text-violet-400 underline underline-offset-2"
                    onClick={() => setSelected(data.results.filter(r => r.products.length > 0).map(r => r.source))}>
                    Show all
                  </button>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {displayed.map((p, i) => (
                    <ProductCard key={`${p.source}-${p.url}-${i}`} product={p} index={i} />
                  ))}
                </div>
              )}
            </motion.div>
        ...
        {!loading && !error && !data && (
          <div className="flex items-center justify-center flex-1 py-20">
            <p className="text-sm" style={{ color: "rgba(240,240,255,0.45)" }}>
