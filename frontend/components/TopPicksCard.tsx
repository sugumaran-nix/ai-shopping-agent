  ...
  return (
    <section className="overflow-hidden rounded-[26px] border border-[#b8d16f] bg-[#f7faed] shadow-[0_18px_50px_rgba(137,173,53,0.1)]" aria-labelledby="top-picks-title">
      <div className="flex flex-col gap-3 border-b border-[#d7e4b5] bg-[#eff7d9] px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
        ...
        <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.14em] text-[#718239]"><ListFilter className="h-3.5 w-3.5" aria-hidden /> Ranked by comparable price</div>
      </div>
      <div className="grid gap-2 p-3 sm:grid-cols-2 sm:p-4 xl:grid-cols-5">{products.map((product, index) => <div key={`${product.source}-${product.url}-${index}`} className="relative"><span className="absolute left-2 top-2 z-10 flex h-5 min-w-5 items-center justify-center rounded-full bg-[#171a16] px-1.5 text-[9px] font-black text-[#c9f36b]" aria-label={`Rank ${index + 1}`}>{index + 1}</span><ProductCard product={product} isLowestPrice={index === 0} showSource /></div>)}</div>
      <p className="border-t border-[#d7e4b5] px-4 py-3 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#718239]">Prices are ranked only when the returned currency is comparable. Open a product to verify delivery and availability.</p>
    </section>
  )
}
