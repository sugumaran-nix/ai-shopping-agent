...
export function ProductCard({ product, isLowestPrice = false }: { product: Product; isLowestPrice?: boolean }) {
  ...
  return (
    <a
      ...
      className={`group flex gap-3 rounded-[18px] border bg-white/85 p-3 transition-all duration-200 hover:-translate-y-0.5 hover:bg-white hover:shadow-[0_10px_28px_rgba(42,53,25,0.08)] active:scale-[0.99] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#9bc64c] focus-visible:ring-offset-2 ${isLowestPrice ? 'border-[#a9ca54] bg-[#fbfff1] shadow-[0_8px_24px_rgba(137,173,53,0.12)] hover:border-[#86ad32]' : 'border-[#e5e7df] hover:border-[#bfd48a]'}`}
      aria-label={`${product.title}, ${price}${isLowestPrice ? ', lowest price found' : ''} — opens in new tab`}
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between py-0.5">
        <div className="flex items-start justify-between gap-2"><p className="line-clamp-2 text-xs font-bold leading-snug text-[#343a31] transition-colors group-hover:text-[#4e6d19]">{product.title}</p>{isLowestPrice && <span className="flex flex-shrink-0 items-center gap-1 rounded-full bg-[#c9f36b] px-2 py-1 text-[9px] font-black uppercase tracking-[0.08em] text-[#35530a]"><Check className="h-3 w-3" aria-hidden /> Best</span>}</div>
        <div className="mt-2 flex items-end justify-between gap-2">
          <div><p className={`text-base font-black tabular-nums tracking-[-0.03em] ${isLowestPrice ? 'text-[#4e6d19]' : 'text-[#171a16]'}`}>{price}</p>{product.rating !== null && <p className="mt-1 flex items-center gap-1 text-[10px] font-semibold tabular-nums text-[#92988e]"><Star className="h-3 w-3 fill-[#e3aa43] text-[#e3aa43]" aria-hidden /><span aria-label={`${product.rating} out of 5 stars`}>{product.rating.toFixed(1)}</span>{product.review_count !== null && <span className="text-[#b6bbb1]">({product.review_count.toLocaleString('en-IN')})</span>}</p>}</div>
