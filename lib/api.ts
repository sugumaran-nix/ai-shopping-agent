...
// Re-export types so components can import from here if needed
export type { SearchResponse, Product, SourceResult, Status, Source } from "@/types";

const API_BASE = process.env.NEXT_PUBLIC_API_BASE_URL;

export const SITE_META: Record<Source, { label: string; color: string; bg: string }> = {
...
export async function searchProducts(
  ...
  try {
    ...
    throw new ApiError(
      "Cannot reach the backend. It may be starting up — wait 20 seconds and retry."
    );
  }
